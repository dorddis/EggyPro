...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: statsData, error: dbError, usedFallback } = await ApiErrorHandler.safeAsync(
      async () => {
        ...
        try {
          ...
          logger.warn('STATS_API', 'Failed to get total products, using fallback', error);
        }
        ...
        try {
          ...
          logger.warn('STATS_API', 'Failed to get total stock, using fallback', error);
        }
        ...
        try {
          ...
          logger.warn('STATS_API', 'Failed to get review stats, using fallback', error);
        }
        ...
        try {
          ...
          logger.warn('STATS_API', 'Failed to get low stock products, using fallback', error);
        }
    ...
    if (dbError && !statsData) {
      logger.warn('STATS_API', 'Database stats failed, using fallback data', { error: dbError.message });
    ...
    logger.warn('STATS_API', 'Critical error, returning fallback stats', error);
    return ApiErrorHandler.createSuccessResponse(mockStats, {
      fallback: true
    });
  }
